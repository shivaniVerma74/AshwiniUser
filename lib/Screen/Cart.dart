...
import 'package:eshop_multivendor/Helper/upi_class.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
              ...
              child: Card(
                ...
                child: InkWell(
                  ...
                  onTap: !saveLater && !context.read<CartProvider>().isProgress
                    ...
                    saveForLater(
                        cartList[index].productList![0].availability == "0" ? cartList[index].productList![0].prVarientList![selectedPos].id! : cartList[index].varientId,"1",
                        cartList[index].productList![0].availability == "0" ? "1" : cartList[index].qty,
                        double.parse(cartList[index].perItemTotal!),
                        cartList[index],false
                    );
                  }: null,
                ),
              ),
                            ...
                            Flexible(
                              child: Row(
                                ...
                                children: <Widget>[
                                  Flexible(
                                    child: Text(
                                      double.parse(cartList[index].productList![0].prVarientList![selectedPos].disPrice!) != 0 ? CUR_CURRENCY! + "" + cartList[index].productList![0].prVarientList![selectedPos].price! : "",
                                      maxLines: 1,
                                      overflow: TextOverflow.ellipsis,
                                      style: Theme.of(context)
                                          .textTheme
  ...
  setSnackbar(
    ...
    ScaffoldMessenger.of(context).showSnackBar(new SnackBar(
      ...
      content: new Text(
        msg,
        textAlign: TextAlign.center,
        style: TextStyle(color: Theme.of(context).colorScheme.black, fontSize: 17),
      ),
  ...
  checkout(List<SectionModel> cartList) {
    print("Cart Total Amount++++++${totalamount.toString()}");
    _razorpay = Razorpay();
                                  ...
                                  onBtnSelected: _placeOrder
                                    ...
                                    if (selAddress == null ||
                                      ...
                                      checkoutState!(() {
                                        ...
                                        MIN_ALLOW_CART_AMT!) > oriPrice) {setSnackbar(
                                        getTranslated(context, 'MIN_CART_AMT')!, _checkscaffoldKey);
                                    }
                                    // else if (!deliverable) {
                                    //   checkDeliverable();
                                    // }
  ...
  doPayment() {
    if (payMethod == getTranslated(context, 'PAYPAL_LBL')) {
      ...
      Navigator.pop(context);
      print("AAAAAAAAAAAAA ${totalPrice}");
      UpiPayment upiPayment = new UpiPayment(totalPrice.toString(), context, (value) {
      ...
      // UpiPaymentClass upiPayment = new Upi(totalPrice.toString(), context, (value) {
      //   print("upi stateus" + value.status.toString());
      //   if(value!=null){
      //     Navigator.pop(context);
      //     Fluttertoast.showToast(msg: "Payment Success");
      //     placeOrder(value);
      //   }
      //   /*if(value.status==UpiTransactionStatus.failure){
      //     Fluttertoast.showToast(msg: "Payment Failed");
      //     print("Upi status..... ${value.status} ${UpiTransactionStatus.success}");
      //   }else{
      //     Navigator.pop(context);
      //     placeOrder('');
      //     print("Upi status...22222.. ${value.status} ${UpiTransactionStatus.success}");
      //   }*/
      // },
     // );
     // upiPayment.init();
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            if (addressList.length == 1) {
              ...
              if (ISFLAT_DEL) {
               // if (totalPrice < double.parse(addressList[0].freeAmt!))
                  ...
                  print("dddddddddddddddddd ${delCharge}");
                // else
                //   delCharge = 0;
              }
            } else {
            ...
            print("Del Charge in get address ${delCharge}");
            totalPrice = totalPrice + delCharge;
            print("totalprice in get address ${totalPrice}");
          } else {
            if (ISFLAT_DEL) {
              if ((oriPrice) < double.parse(MIN_AMT!))
                delCharge = double.parse(CUR_DEL_CHR!);
    ...
    if (_isNetworkAvail) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          bool error = getdata["error"];
          String? msg = getdata["message"];
          print("final response here now ${error} and  ${payMethod} and  ${getdata}");
          if (!error) {
  ...
  Future<void> addTransaction(String? tranId, String orderID, String? status,
    ...
    try {
      ...
      print("transitional hee ${parameter}");
      Response response =
                ...
                children: [
                  ...
                  Text(
                    addressList[selectedAddress!].address! +
                        ", " +
                        addressList[selectedAddress!].cityId! +
                        ", " +
  ...
  Future<void> validatePromo(bool check) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("Promo code parameter ${parameter}");
        Response response =
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            var data = getdata["data"];
            totalPrice = double.parse(data["final_total"]) + delCharge;
            print("totallllllllllllllll ${totalPrice}");
            print("Final Total nowwww ${promoAmt}");
            setSnackbar(msg!, _checkscaffoldKey);
          }
          if (isUseWallet!) {
            remWalBal = 0;
                              ...
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      "$CUR_CURRENCY ${totalPrice.toString()}",
