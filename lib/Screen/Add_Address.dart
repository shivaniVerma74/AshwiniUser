...
String? latitude, longitude, state, country , city ,area;
...
class StateAddress extends State<AddAddress> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    stateC = new TextEditingController();
    cityC = new TextEditingController();
    areaC = new TextEditingController();
    countryC = new TextEditingController();
    landmarkC = new TextEditingController();
    ...
    if (widget.update!) {
      ...
      addressC!.text = item.address!;
      stateC!.text = item.state!;
      cityC!.text = item.cityId!;
      areaC!.text = item.areaId!;
      countryC!.text = item.country!;
      latitude = item.latitude;
      longitude = item.longitude;
              ...
              content: Column(
                ...
                children: [
                  Padding(
                    padding: EdgeInsets.fromLTRB(20.0, 20.0, 0, 0),
                    child: Text(
                      getTranslated(context, 'STATESELECT_LBL')!,
                      style: Theme.of(this.context)
                          .textTheme
                          .subtitle1!
                          .copyWith(
    ...
    return citySearchLIst
        ...
        InkWell(
          onTap: () {
            ...
            print("this is my state id ${city}");
  ...
  Future<void> getCities() async {
    print("${getCitiesApi}____________________________");
    try {
      Response response = await post(getCitiesApi, headers: headers)
          .timeout(Duration(seconds: timeOut));
      print("${getCitiesApi}____________________________");
      ...
      if (!error) {
        ...
        print("$data}____________________________");
  ...
  Future<void> addNewAddress() async {
    ...
    var headers = {
      'Cookie': 'ci_session=0d06bf7a101792983a08f292ca5955495c825ad1'
    };
    var request = http.MultipartRequest('POST',

     widget.update! == true ? Uri.parse('https://ashwiniamruttulya.co.in/app/v1/api/update_address') : Uri.parse('https://ashwiniamruttulya.co.in/app/v1/api/add_address')
    );
    ...
    if (response.statusCode == 200) {
      ...
      final getdata = json.decode(finalResult);
        bool error = getdata["error"];
        String? msg = getdata["message"];
        await buttonController!.reverse();
        if (!error) {
    ...
    //   print("para here ${data.toString()}");
    //   if (widget.update!) data[ID] = addressList[widget.index!].id;
    //   print("mmm ${widget.update!} and working");
    //   Response response = await http.post(
    //  //      widget.update! ?
    //  //      updateAddressApi
    //  // :
    //       Uri.parse(baseUrl + "add_address"),
    //           //: getAddAddressApi,
    //       body: data,
    //       headers: headers)
    //       .timeout(Duration(seconds: timeOut));
